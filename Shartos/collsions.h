...
// static geometry and dynamic collisionObjects.
//
...
struct CollisionData
{
	...
	object *collisionObject; // Pointer to the collisionObject to perform collision detection with.
...
//-----------------------------------------------------------------------------
// Perfrom collision detection between the give collisionObject and the scene.
//-----------------------------------------------------------------------------
inline void CollideWithScene( CollisionData *data, Vertex *vertices, SceneFace *faces, unsigned long totalFaces, LinkedList< SceneObject > *collisionObjects, unsigned long recursion = 5 )
{
	...
	for( unsigned long f = 0; f < totalFaces; f++ )
	{
		...
		vertex0.x = vertices[faces[f].vertex0].translation.x / data->collisionObject->GetEllipsoidRadius().x;
		vertex0.y = vertices[faces[f].vertex0].translation.y / data->collisionObject->GetEllipsoidRadius().y;
		vertex0.z = vertices[faces[f].vertex0].translation.z / data->collisionObject->GetEllipsoidRadius().z;
		vertex1.x = vertices[faces[f].vertex1].translation.x / data->collisionObject->GetEllipsoidRadius().x;
		vertex1.y = vertices[faces[f].vertex1].translation.y / data->collisionObject->GetEllipsoidRadius().y;
		vertex1.z = vertices[faces[f].vertex1].translation.z / data->collisionObject->GetEllipsoidRadius().z;
		vertex2.x = vertices[faces[f].vertex2].translation.x / data->collisionObject->GetEllipsoidRadius().x;
		vertex2.y = vertices[faces[f].vertex2].translation.y / data->collisionObject->GetEllipsoidRadius().y;
		vertex2.z = vertices[faces[f].vertex2].translation.z / data->collisionObject->GetEllipsoidRadius().z;

		// Check for collision with this face.
		CheckFace( data, vertex0, vertex1, vertex2 );
	}

	// Create a list of hit ghost collisionObjects and a list of the distances to them.
	LinkedList< SceneObject > *ghostHits = new LinkedList< SceneObject >;
	LinkedList< float > *ghostDistances = new LinkedList< float >;

	// Variables used for the following collisionObject collision check.
	D3DXVECTOR3 translation, velocity, vectorColliderObject, vectorObjectCollider, vectorObjectRadius;
	float distToCollision, colliderRadius, collisionObjectRadius;

	// Go through the list of collisionObjects.
	SceneObject *hitObject = NULL;
	SceneObject *nextObject = collisionObjects->GetFirst();
	while( nextObject != NULL )
	{
		// Skip this collisionObject if it is the collider. It can't check against itself.
		if( nextObject != data->collisionObject )
		{
			// Get the translation and velocity of this collisionObject in ellipsoid space.
			translation.x = nextObject->GetTranslation().x / data->collisionObject->GetEllipsoidRadius().x;
			translation.y = nextObject->GetTranslation().y / data->collisionObject->GetEllipsoidRadius().y;
			translation.z = nextObject->GetTranslation().z / data->collisionObject->GetEllipsoidRadius().z;
			velocity.x = nextObject->GetVelocity().x / data->collisionObject->GetEllipsoidRadius().x;
			velocity.y = nextObject->GetVelocity().y / data->collisionObject->GetEllipsoidRadius().y;
			velocity.z = nextObject->GetVelocity().z / data->collisionObject->GetEllipsoidRadius().z;
			velocity *= data->elapsed;

			// Get the normalized vectors from the collider to this collisionObject and vice versa.
			...
			vectorObjectRadius.x = vectorObjectCollider.x * nextObject->GetEllipsoidRadius().x / data->collisionObject->GetEllipsoidRadius().x;
			vectorObjectRadius.y = vectorObjectCollider.y * nextObject->GetEllipsoidRadius().y / data->collisionObject->GetEllipsoidRadius().y;
			vectorObjectRadius.z = vectorObjectCollider.z * nextObject->GetEllipsoidRadius().z / data->collisionObject->GetEllipsoidRadius().z;
			collisionObjectRadius = D3DXVec3Length( &vectorObjectRadius );

			// Check for collision between the two spheres.
			if( IsSphereCollidingWithSphere( &distToCollision, data->translation, translation, velocity - data->velocity, colliderRadius + collisionObjectRadius ) == true )
			{
				// Check if the hit collisionObject is a ghost.
				if( nextObject->GetGhost() == true )
				{
					// If both collisionObject's are allowed to register collisions, then store a pointer to the hit collisionObject and the distance to hit it.
					if( nextObject->GetIgnoreCollisions() == false && data->collisionObject->GetIgnoreCollisions() == false )
					{
				...
				else
				{
					...
					if( data->collisionFound == false || distToCollision < data->distance )
					{
						...
						// Store a pointer to the hit collisionObject.
		...
		// Go to the next collisionObject.
		nextObject = collisionObjects->GetNext( nextObject );
	}

	// Iterate through the list of hit ghost collisionObjects and their collision distances.
	ghostHits->Iterate( true );
	ghostDistances->Iterate( true );
	while( ghostHits->Iterate() != NULL && ghostDistances->Iterate() != NULL )
	{
		// If the distance to hit the ghost collisionObject is less than the distance to the closets real collision, then the ghost collisionObject has been hit.
		if( *ghostDistances->GetCurrent() < data->distance )
		{
			// Register the collision between both collisionObjects.
			ghostHits->GetCurrent()->CollisionOccurred( data->collisionObject, data->frameStamp );
			data->collisionObject->CollisionOccurred( ghostHits->GetCurrent(), data->frameStamp );
		}
	}
	...
	// Calculate the destination (i.e. the point the collisionObject was trying to get to).
	D3DXVECTOR3 destination = data->translation + data->velocity;

	// The new translation will be the point where the collisionObject actually ends up.
	D3DXVECTOR3 newTranslation = data->translation;

	// Ignore the movement if the collisionObject is already very close to its destination.
	if( data->distance >= epsilon )
	{
		...
		// the collisionObject does not move right up to the actual intersection point.
		D3DXVec3Normalize( &newVelocity, &newVelocity );
		data->intersection = data->intersection - newVelocity * epsilon;
	}

	// Check if the collision occured with an collisionObject.
	if( hitObject != NULL )
	{
		// Set the new translation of the collisionObject.
		data->translation = newTranslation;

		// Calculate and apply a push velocity so collisionObjects can push one another.
		D3DXVECTOR3 push = ( hitObject->GetVelocity() + data->collisionObject->GetVelocity() ) / 10.0f;
		hitObject->SetVelocity( push );
		data->collisionObject->SetVelocity( push );

		// Register the collision between both collisionObjects, if thay are allowed.
		if( hitObject->GetIgnoreCollisions() == false && data->collisionObject->GetIgnoreCollisions() == false )
		{
			hitObject->CollisionOccurred( data->collisionObject, data->frameStamp );
			data->collisionObject->CollisionOccurred( hitObject, data->frameStamp );
		}
	...
	recursion--;
	if( recursion > 0 )
		CollideWithScene( data, vertices, faces, totalFaces, collisionObjects, recursion );
}
...
{
	// Calculate the collisionObject's translation in ellipsoid space.
	data->translation.x = data->collisionObject->GetTranslation().x / data->collisionObject->GetEllipsoidRadius().x;
	data->translation.y = data->collisionObject->GetTranslation().y / data->collisionObject->GetEllipsoidRadius().y;
	data->translation.z = data->collisionObject->GetTranslation().z / data->collisionObject->GetEllipsoidRadius().z;

	// Calculate the collisionObject's velocity in ellipsoid space.
	data->velocity = data->collisionObject->GetVelocity() * data->elapsed;
	data->velocity.x /= data->collisionObject->GetEllipsoidRadius().x;
	data->velocity.y /= data->collisionObject->GetEllipsoidRadius().y;
	data->velocity.z /= data->collisionObject->GetEllipsoidRadius().z;
	...
	data->velocity.x = data->gravity.x / data->collisionObject->GetEllipsoidRadius().x;
	data->velocity.y = data->gravity.y / data->collisionObject->GetEllipsoidRadius().y;
	data->velocity.z = data->gravity.z / data->collisionObject->GetEllipsoidRadius().z;

	// Perform another recursive collision detection to apply gravity.
	CollideWithScene( data, vertices, faces, totalFaces, dynamicObjects );

	// Convert the collisionObject's new translation back out of ellipsoid space.
	data->translation.x = data->translation.x * data->collisionObject->GetEllipsoidRadius().x;
	data->translation.y = data->translation.y * data->collisionObject->GetEllipsoidRadius().y;
	data->translation.z = data->translation.z * data->collisionObject->GetEllipsoidRadius().z;
	...
	for( unsigned long f = 0; f < totalFaces; f++ )
	{
		...
		// Preform a ray intersection test to see if this face is under the collisionObject.
		float distance;
	...
	// of the collisionObject along the y axis, then the collisionObject is embedded in
	// the ground. So just push the collisionObject up out of the ground.
	if( hitDistance < data->collisionObject->GetEllipsoidRadius().y )
		data->translation.y += data->collisionObject->GetEllipsoidRadius().y - hitDistance;

	// Check if the collisionObject is touching the ground.
	if( hitDistance < data->collisionObject->GetEllipsoidRadius().y + 0.1f / data->scale )
		data->collisionObject->SetTouchingGroundFlag( true );
	else
		data->collisionObject->SetTouchingGroundFlag( false );

	// Update the collisionObject's translation after collision detection.
	data->collisionObject->SetTranslation( data->translation );
}
